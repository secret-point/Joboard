...
export default class RequisitionService {
  ...
  constructor() {
    this.axiosInstance = axiosHelper("/api/requisition", { "Cache-Control": "no-cache" });
  }

  async getDisqualifiedQuestions( requisitionId: string ) {
    const response = await this.axiosInstance.get(`/disqualified-questions/${requisitionId}`);
    return response.data;
  }

  async getRequisitionHeaderInfo( requisitionId: string ) {
    const response = await this.axiosInstance.get(`/header-info/${requisitionId}`);
    return response.data;
  }
  ...
  async getJobDescription( childRequisitionId: string ) {
    const response = await this.axiosInstance.get(`/job-description/${childRequisitionId}`);
  ...
  async getShiftPreferenceDetails( applicationId: string, requisitionId: string ) {
    const response = await this.axiosInstance.get(`/get-shift-preference-details/${applicationId}/${requisitionId}`);
