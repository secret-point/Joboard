...
import { Error403Page } from "../page-list";
...
describe("Test <RedirectPage/>", () => {
      ...
      <Provider store={store}>
        <StencilProvider>
          <MemoryRouter initialEntries={["/403"]}>
            <Route path="/403">
              {isPageLoadFromList ? <Error403Page /> : <Error403 />}
            </Route>
          </MemoryRouter>
        </StencilProvider>
      </Provider>
  ...
  test("should load with 403 page", () => {
  ...
  test("should load with 403 page with out loading", () => {
