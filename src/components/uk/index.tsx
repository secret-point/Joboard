...
import DragonStoneAppUK from "./dspApp";
...
getInitialData()
  .then((data: any) => {
    ...
    const isNewBBFlow = isNewBBuiPath(page, newBBUIPathName.UK);
    ...
    if (!isNewBBFlow) {
      ...
      const redirectUrl = `${newBBUIPathName.UK}${requestQueryString}#/${appHashUrl}`;
          ...
          <StencilProvider>
            <Col gridGap="m" padding="n">
              ...
              >
                <MainWithSkipLink>
                  <Switch>
                    <Route path={newBBUIPathName.UK}>
                      <DragonStoneAppUK />
                    </Route>
                    <Route path="/" render={() => (
                      <Redirect to={{
                        pathname: newBBUIPathName.UK,
